 ...
 *
 * - Reads books from artifacts/catalog/sci-fi.ts (default)
 * - Computes expected dst: covers/<id>.webp
 * - Finds existing matching file by slug of title in filename (best-effort)
 ...
 * --catalog=<ts>  Catalog module path (default: artifacts/catalog/sci-fi.ts)
 */
import { $ } from "bun";
...
const catalogPath = (args.get("catalog") as string) || path.resolve("artifacts/catalog/sci-fi.ts");
...
async function loadCatalog(): Promise<Array<{ id: string; title: string }>> {
  const mod = await import(pathToFileURL(catalogPath).toString());
  const list = (mod.default ?? mod.sciFiCatalogWithIdCovers ?? mod.sciFiCatalog) as any[];
  return list.map((b) => ({ id: String(b.id), title: String(b.title) }));
}
...
async function main() {
  ...
  for (const b of catalog) {
    ...
    const candidate = findCandidate(files, b.title);
