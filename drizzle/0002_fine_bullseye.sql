...
CREATE INDEX `authors_sort_name_idx` ON `authors` (`sort_name`);--> statement-breakpoint
CREATE TABLE `catalog_change_events` (
	...
	CONSTRAINT "catalog_change_events_json_ck" CHECK(json_valid("catalog_change_events"."payload_json")),
...
--> statement-breakpoint
CREATE INDEX `catalog_change_events_type_created_idx` ON `catalog_change_events` (`change_type`,`created_at`);--> statement-breakpoint
...
CREATE TABLE `cover_assets` (
	...
	FOREIGN KEY (`source_policy_id`) REFERENCES `metadata_sources`(`id`) ON UPDATE no action ON DELETE set null,
	CONSTRAINT "cover_assets_object_key_ck" CHECK(length(trim("cover_assets"."object_key")) > 0 and substr("cover_assets"."object_key", 1, 8) = '/covers/'),
...
--> statement-breakpoint
CREATE UNIQUE INDEX `cover_assets_object_key_uq` ON `cover_assets` (`object_key`);--> statement-breakpoint
CREATE TABLE `edition_covers` (
	`edition_id` text NOT NULL,
	`cover_asset_id` text NOT NULL,
	`position` integer NOT NULL,
	`is_primary` integer DEFAULT false NOT NULL,
	PRIMARY KEY(`edition_id`, `cover_asset_id`),
	FOREIGN KEY (`edition_id`) REFERENCES `editions`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`cover_asset_id`) REFERENCES `cover_assets`(`id`) ON UPDATE no action ON DELETE restrict,
	CONSTRAINT "edition_covers_position_ck" CHECK("edition_covers"."position" >= 0)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `edition_covers_position_uq` ON `edition_covers` (`edition_id`,`position`);--> statement-breakpoint
CREATE UNIQUE INDEX `edition_covers_primary_uq` ON `edition_covers` (`edition_id`) WHERE "edition_covers"."is_primary" = 1;--> statement-breakpoint
CREATE INDEX `edition_covers_owner_position_idx` ON `edition_covers` (`edition_id`,`position`);--> statement-breakpoint
CREATE INDEX `edition_covers_primary_lookup_idx` ON `edition_covers` (`edition_id`,`is_primary`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `edition_identifiers_value_uq` ON `edition_identifiers` (`scheme`,`value_normalized`);--> statement-breakpoint
CREATE UNIQUE INDEX `edition_identifiers_primary_uq` ON `edition_identifiers` (`edition_id`) WHERE "edition_identifiers"."is_primary" = 1;--> statement-breakpoint
CREATE INDEX `edition_identifiers_owner_idx` ON `edition_identifiers` (`edition_id`);--> statement-breakpoint
CREATE TABLE `edition_languages` (
	`edition_id` text NOT NULL,
	`language_tag` text NOT NULL,
	`position` integer NOT NULL,
	PRIMARY KEY(`edition_id`, `language_tag`),
	FOREIGN KEY (`edition_id`) REFERENCES `editions`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`language_tag`) REFERENCES `languages`(`tag`) ON UPDATE no action ON DELETE restrict,
	CONSTRAINT "edition_languages_position_ck" CHECK("edition_languages"."position" >= 0)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `edition_languages_position_uq` ON `edition_languages` (`edition_id`,`position`);--> statement-breakpoint
CREATE INDEX `edition_languages_owner_position_idx` ON `edition_languages` (`edition_id`,`position`);--> statement-breakpoint
CREATE INDEX `edition_languages_language_edition_idx` ON `edition_languages` (`language_tag`,`edition_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `edition_publishers_position_uq` ON `edition_publishers` (`edition_id`,`position`);--> statement-breakpoint
CREATE INDEX `edition_publishers_owner_position_idx` ON `edition_publishers` (`edition_id`,`position`);--> statement-breakpoint
...
CREATE INDEX `editions_publication_sort_work_idx` ON `editions` (`publication_sort_date`,`work_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `entity_aliases_target_idx` ON `entity_aliases` (`entity_type`,`to_id`);--> statement-breakpoint
CREATE TABLE `field_observations` (
	...
	CONSTRAINT "field_observations_json_ck" CHECK(json_valid("field_observations"."value_json") and ("field_observations"."parent_ids_json" is null or json_valid("field_observations"."parent_ids_json"))),
...
CREATE TABLE `field_resolution_heads` (
	...
	FOREIGN KEY (`resolution_id`) REFERENCES `field_resolutions`(`id`) ON UPDATE no action ON DELETE restrict,
...
CREATE TABLE `field_resolutions` (
	...
	FOREIGN KEY (`selected_observation_id`) REFERENCES `field_observations`(`id`) ON UPDATE no action ON DELETE restrict,
	FOREIGN KEY (`previous_resolution_id`) REFERENCES `field_resolutions`(`id`) ON UPDATE no action ON DELETE restrict,
...
CREATE TABLE `metadata_sources` (
	...
	CONSTRAINT "metadata_sources_policy_json_ck" CHECK(json_valid("metadata_sources"."metadata_policy") and json_valid("metadata_sources"."asset_policy")),
...
--> statement-breakpoint
CREATE UNIQUE INDEX `metadata_sources_key_uq` ON `metadata_sources` (`key`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `source_record_links_entity_idx` ON `source_record_links` (`entity_type`,`entity_id`,`state`);--> statement-breakpoint
CREATE TABLE `source_records` (
	...
	`state` text NOT NULL,
	FOREIGN KEY (`source_id`) REFERENCES `metadata_sources`(`id`) ON UPDATE no action ON DELETE restrict,
	...
	CONSTRAINT "source_records_payload_json_ck" CHECK("source_records"."payload_json" is null or json_valid("source_records"."payload_json")),
...
--> statement-breakpoint
CREATE UNIQUE INDEX `source_records_source_key_uq` ON `source_records` (`source_id`,`record_key`);--> statement-breakpoint
CREATE INDEX `source_records_refresh_idx` ON `source_records` (`source_id`,`state`,`retrieved_at`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `work_authors_position_uq` ON `work_authors` (`work_id`,`position`);--> statement-breakpoint
CREATE INDEX `work_authors_owner_position_idx` ON `work_authors` (`work_id`,`position`);--> statement-breakpoint
CREATE INDEX `work_authors_author_work_idx` ON `work_authors` (`author_id`,`work_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `work_categories_position_uq` ON `work_categories` (`work_id`,`position`);--> statement-breakpoint
CREATE UNIQUE INDEX `work_categories_primary_uq` ON `work_categories` (`work_id`) WHERE "work_categories"."is_primary" = 1;--> statement-breakpoint
CREATE INDEX `work_categories_owner_position_idx` ON `work_categories` (`work_id`,`position`);--> statement-breakpoint
CREATE INDEX `work_categories_category_work_idx` ON `work_categories` (`category_id`,`work_id`);--> statement-breakpoint
CREATE TABLE `works` (
	`id` text PRIMARY KEY NOT NULL,
	...
	`created_at` integer NOT NULL,
	`updated_at` integer NOT NULL,
	FOREIGN KEY (`preferred_edition_id`) REFERENCES `editions`(`id`) ON UPDATE no action ON DELETE set null,
	CONSTRAINT "works_titles_nonempty_ck" CHECK(length(trim("works"."preferred_title")) > 0 and length(trim("works"."sort_title")) > 0)
);
--> statement-breakpoint
CREATE INDEX `works_sort_title_id_idx` ON `works` (`sort_title`,`id`);--> statement-breakpoint
CREATE INDEX `works_preferred_edition_idx` ON `works` (`preferred_edition_id`);
